...
import { useLoginMutation } from '../Slice/UserApiSlice'; // Import the login mutation
...
const UserLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await login({ email, password }).unwrap(); // Ensure correct API response
      console.log("API Response:", response); // Log the full response to see its structure
      const { user, accessToken } = response;// Ensure correct API response
      dispatch(setCredentials({ user, token: accessToken })); // Use correct keys for the dispatch
      navigate('/products'); // Redirect on successful login
    } catch (error) {
      console.error("Login failed:", error); // Log the error
      toast.error(error?.data?.message || 'Login failed'); // Safely handle the error message
    } finally {
