...
const UserRegistration = () => {
  ...
  const handleRegister = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await register({ name, email, password, address, phone }).unwrap(); // Register the user
      console.log('Registration Successful:', response);
      toast.success('Registration successful!'); // Success notification
      navigate('/login'); // Redirect to login page
    } catch (error) {
      console.error('Registration failed:', error);
      toast.error(error?.data?.message || 'Registration failed'); // Display error message
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="bg-gray-100 flex rounded-2xl shadow-lg max-w-4xl p-5 items-center">
          <div className="md:w-1/2 px-8 md:px-16">
            ...
            <form className="flex flex-col gap-4" onSubmit={handleRegister}>
              ...
              <textarea
                className="p-2 rounded-xl border resize-none bg-white text-black"
                name="address"
                placeholder="Address"
                value={address}
          ...
          <div className="md:block hidden w-1/2">
            <img className="rounded-2xl" src={pic1} alt="Registration" />
          </div>
        </div>
      </section>
      <Footer />
